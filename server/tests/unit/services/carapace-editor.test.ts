...
describe('CarapaceEditor', () => {
  ...
  beforeEach(() => {
    // 创建临时 carapace.md 文件（放在 tmpdir 下，在 CLAWBUDS_DATA_DIR 允许范围内）
    ...
    // CRITICAL-3: tmpdir 处于 homedir 下（或设置 CLAWBUDS_DATA_DIR = tmpdir()）
    process.env['CLAWBUDS_DATA_DIR'] = tmpdir()
    editor = new CarapaceEditor(mockHistoryRepo, carapacePath)
  })
  ...
  describe('constructor', () => {
    it('should reject path outside CLAWBUDS_DATA_DIR', () => {
      process.env['CLAWBUDS_DATA_DIR'] = tmpdir()
      expect(() => new CarapaceEditor(mockHistoryRepo, '/etc/passwd')).toThrow(
        /must be under/,
      )
    })

    it('should accept path within CLAWBUDS_DATA_DIR', () => {
      process.env['CLAWBUDS_DATA_DIR'] = tmpdir()
      expect(() => new CarapaceEditor(mockHistoryRepo, carapacePath)).not.toThrow()
    })
  })
  ...
  describe('allow', () => {
    it('should append an allow rule to carapace.md', async () => {
      await editor.allow(clawId, 'alice', '日常梳理消息')
    ...
    it('should not remove existing content', async () => {
      ...
      await editor.allow(clawId, 'bob', '日常消息')
  ...
  describe('escalate', () => {
    it('should append an escalate rule to carapace.md', async () => {
      await editor.escalate(clawId, '当 Pearl 涉及敏感领域时', '需要人工审阅')
      const content = readFileSync(carapacePath, 'utf-8')
      expect(content).toContain('当 Pearl 涉及敏感领域时')
    })

    it('should include Micro-Molt annotation comment', async () => {
      await editor.escalate(clawId, 'condition', 'action')
      const content = readFileSync(carapacePath, 'utf-8')
      expect(content).toContain('<!-- Micro-Molt:')
    })
  ...
  describe('applyMicroMolt', () => {
    it('should apply an allow-type suggestion', async () => {
      ...
      await editor.applyMicroMolt(clawId, suggestion)
      const content = readFileSync(carapacePath, 'utf-8')
      expect(content).toContain('alice')
    })
  ...
  describe('restoreVersion', () => {
    it('should restore content from history record', async () => {
      ...
      await editor.restoreVersion(clawId, 3)
      const content = readFileSync(carapacePath, 'utf-8')
    ...
    it('should backup current version before restoring', async () => {
      ...
      await editor.restoreVersion(clawId, 2)
      expect(mockHistoryRepo.create).toHaveBeenCalledWith(
    ...
    it('should throw if version not found', async () => {
      ...
      await expect(editor.restoreVersion(clawId, 999)).rejects.toThrow()
    })
  })
})
